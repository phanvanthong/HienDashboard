<!-- Tab2: Biến động số lượng sale hoạt động theo tháng -->
<template>
  <ChartCard title="Số lượng sale hoạt động" subtitle="Số nhân viên: Sale">
...
const chartData = computed(() => ({
  ...
  datasets: [{
    ...
    data: (props.byMonth || []).map(d => d.activeSales),
    borderColor: '#0071e3',
...
const chartOptions = computed(() => ({
  ...
  plugins: {
    ...
    tooltip: {
      ...
      padding: 10,
      cornerRadius: 8,
      callbacks: { label: c => ` ${c.raw} sale hoạt động` },
    },
  },
}))
</script>
